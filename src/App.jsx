...
const soCustomers = ["All Customers", "WHITECAP (All Branches)",
  ...Array.from(new Set(soLines.map(r => r.Customer).filter(c => !c.toUpperCase().replace(" ","").startsWith("WHITECAP")))).sort()];
...
export default function Dashboard() {
  ...
  const filteredSOLines = useMemo(() => soLines.filter(r => {
    ...
    if (soCustomer !== "All Customers") {
      if (soCustomer === "WHITECAP (All Branches)") {
        // Match any WHITE CAP or WHITECAP variant
        if (!r.Customer.toUpperCase().replace(" ","").startsWith("WHITECAP")) return false;
      } else {
        if (r.Customer !== soCustomer) return false;
      }
    }
  ...
  const customerBar = useMemo(() => {
    ...
    filteredSOLines.forEach(r => {
      // Group WHITE CAP / WHITECAP variants into one group
      const isWhiteCap = r.Customer.toUpperCase().replace(" ","").startsWith("WHITECAP");
      const groupKey = isWhiteCap ? "WHITECAP (All Branches)" : r.Customer;
      if (!map[groupKey]) map[groupKey] = { customer: isWhiteCap ? "WHITECAP" : r.Customer.split(" ").slice(0,2).join(" "), full: groupKey, isGroup: isWhiteCap, openAmt: 0 };
      map[groupKey].openAmt += r["Open Amt"];
    });
        ...
        {activeTab === "so" && (<div>
          ...
          <div style={{ background: "#0d1a30", borderRadius: 10, padding: 18, border: "1px solid #1e3a5f", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 10, color: "#4a6a88" }}>
                <span style={{ color: "#f97316", fontWeight: 700 }}>&#9135; Total Open SOs</span>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={200}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginBottom: 20 }}>
            <div style={{ background: "#0d1a30", borderRadius: 10, padding: 18, border: "1px solid #1e3a5f" }}>
              ...
              <div style={{ display: "flex", alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  {soAgingSummary.map(d => (
                    <div key={d.bucket} onClick={() => setSoBucket(soBucket === d.bucket ? null : d.bucket)}
                      style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 7, cursor: "pointer", opacity: soBucket && soBucket !== d.bucket ? 0.4 : 1 }}>
                      <div style={{ width: 10, height: 10, borderRadius: 2, background: BCOLORS[d.bucket], flexShrink: 0 }} />
                      <div>
                        <div style={{ fontSize: 10, color: "#8aa8c8", fontWeight: 600 }}>{d.bucket}</div>
                        <div style={{ fontSize: 11, color: "#c8d6e8", fontWeight: 700 }}>{d.value} SOs</div>
                      </div>
                    </div>
                  ))}
                </div>
